...
function DisclaimerBar({ compact }: { compact?: boolean }) {
  return (<div className="flex gap-3 items-start rounded-lg border px-4 py-3" style={{ backgroundColor: AMBER_BG, borderColor: AMBER_BORDER }}>
...
function Row({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  return (<div className="flex justify-between text-sm"><span style={{ color: INK_SOFT }}>{label}</span><span style={{ color: INK, fontWeight: bold ? 700 : 500 }}>{value}</span></div>);
}

function Field({ label, value, onChange, full, placeholder }: any) {
  return (<label className={`text-sm ${full ? "sm:col-span-2" : ""}`}>
...
export default function GeradorMaterialJuridico() {
  ...
  // ✅ NOVO: Lê o parâmetro ?perfil= da URL e pré-seleciona o perfil
  useEffect(() => {
  ...
  function gerarPeticao() {
    ...
    const argumentosFinais = [
      ...
      ...ARGUMENTOS_CONDICIONAIS.filter((a) => argsCondSel[a.id]).map((a, i) => { const numRomano = perfil === "ludopatia" ? ["IX", "X"][i] : ["VIII", "IX"][i]; return { titulo: `${numRomano} — ${a.label.toUpperCase()}`, texto: a.texto(dataFato, horaFato) }; }),
    ];
    let textoRoteiro = "";
        ...
        <div className="grid grid-cols-1 md:grid-cols-[240px_1fr] gap-6">
          ...
          <div className="rounded-xl border p-6" style={{ backgroundColor: PAPER, borderColor: PAPER_LINE }}>
            {tab === "perfil" && (<div>
              ...
              <div className="space-y-4">
                <label className={`flex items-start gap-4 p-4 rounded-lg border cursor-pointer transition-all ${perfil === "generico" ? "border-[#8A6D3B] bg-[#FBF9F4] ring-2 ring-[#8A6D3B]" : "border-[#E4DFD1] hover:bg-white"}`} onClick={() => setPerfil("generico")}>
                  ...
                  <div><span className="text-sm font-bold block" style={{ color: INK }}>Autoexclusão Genérica (Controle Financeiro/Prevenção)</span><p className="text-xs mt-1" style={{ color: INK_SOFT }}>Foco na quebra de contrato regulatório, falha no dever de bloqueio e responsabilidade objetiva da operadora.</p></div>
                </label>
                <label className={`flex items-start gap-4 p-4 rounded-lg border cursor-pointer transition-all ${perfil === "ludopatia" ? "border-[#8A6D3B] bg-[#FBF9F4] ring-2 ring-[#8A6D3B]" : "border-[#E4DFD1] hover:bg-white"}`} onClick={() => setPerfil("ludopatia")}>
                  ...
                  <div><span className="text-sm font-bold block" style={{ color: INK }}>Ludopatia / Saúde Mental (Vulnerabilidade)</span><p className="text-xs mt-1" style={{ color: INK_SOFT }}>Inclui argumentos sobre nulidade das apostas pela Lei 14.790/2023 (art. 26), consumidor hipervulnerável e jurisprudência real. <strong>Laudo não é obrigatório, mas fortalece muito o caso (obtenha de graça no CAPS).</strong></p></div>
                </label>
              </div>
            ...
            {tab === "roteiro" && (<div>
              ...
              <div className="space-y-4 mb-6">
                <label className="flex items-start gap-3 p-3 rounded-lg border cursor-pointer hover:bg-white" style={{ borderColor: PAPER_LINE }}>
                  ...
                  <div className="flex-1"><span className="text-sm font-medium" style={{ color: INK }}>Tentei contato via Chat/Suporte</span>{tentativaChat && <input type="text" placeholder="Número do Protocolo" value={protocoloChat} onChange={(e) => setProtocoloChat(e.target.value)} className="mt-2 w-full px-3 py-2 text-sm rounded-lg border outline-none focus:ring-2" style={{ borderColor: PAPER_LINE }} />}</div>
                </label>
                <label className="flex items-start gap-3 p-3 rounded-lg border cursor-pointer hover:bg-white" style={{ borderColor: PAPER_LINE }}>
                  ...
                  <div className="flex-1"><span className="text-sm font-medium" style={{ color: INK }}>Abri reclamação no Consumidor.gov.br</span>{tentativaConsumidorGov && <input type="text" placeholder="Número do Protocolo da Reclamação" value={protocoloConsumidorGov} onChange={(e) => setProtocoloConsumidorGov(e.target.value)} className="mt-2 w-full px-3 py-2 text-sm rounded-lg border outline-none focus:ring-2" style={{ borderColor: PAPER_LINE }} />}</div>
                </label>
              </div>
            ...
            {tab === "provas" && (<div>
              ...
              <div className="space-y-1 mb-6">{PROVAS.map((item) => { const on = !!checked[item.id]; return (<button key={item.id} onClick={() => setChecked((c) => ({ ...c, [item.id]: !c[item.id] }))} className="w-full flex items-start gap-3 text-left px-3 py-2.5 rounded-lg hover:bg-[#F2EFE6] transition-colors">{on ? <CheckCircle2 size={19} className="shrink-0 mt-0.5" style={{ color: "#3F6B4A" }} /> : <Circle size={19} className="shrink-0 mt-0.5" style={{ color: "#B9B2A0" }} />}<span className="text-sm" style={{ color: INK, opacity: on ? 0.6 : 1, textDecoration: on ? "line-through" : "none" }}>{item.texto}</span></button>); })}</div>
            ...
            {tab === "calculo" && (<div>
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-4"><Field label="Data da Autoexclusão Centralizada" value={dataAutoexclusao} onChange={setDataAutoexclusao} /><Field label="Data do Depósito Indevido" value={dataFato} onChange={setDataFato} /></div>
              <div className="grid sm:grid-cols-2 gap-4 mb-4"><Field label="Valor Perdido / Depositado (R$)" value={valorPerdido} onChange={setValorPerdido} placeholder="Ex: 800" /><Field label="Hora aproximada do depósito" value={horaFato} onChange={setHoraFato} placeholder="Ex: 06:02" /></div>
              <div className="space-y-3 mb-6">
                <label className="flex items-start gap-2 text-sm cursor-pointer p-3 rounded-lg border" style={{ borderColor: PAPER_LINE }}><input type="checkbox" checked={dobro} onChange={(e) => setDobro(e.target.checked)} className="mt-1 w-4 h-4" /><div><span style={{ color: INK, fontWeight: 500 }}>Pedir restituição em DOBRO (art. 42, parágrafo único, CDC)</span><p className="text-xs mt-1" style={{ color: INK_SOFT }}>A restituição em dobro só é devida quando a empresa agiu de má-fé ou negligência grave, sem "engano justificável".</p></div></label>
                <label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={danoMoral} onChange={(e) => setDanoMoral(e.target.checked)} className="w-4 h-4" /><span style={{ color: INK }}>Incluir pedido de Danos Morais</span></label>
              ...
              {calc.excedeLimite ? (<div className="mb-4 p-4 rounded-lg border border-red-300 bg-red-50 text-red-800 text-sm"><div className="flex items-start gap-2"><AlertCircle size={18} className="shrink-0 mt-0.5" /><div><strong>Atenção: Valor acima do limite para Jus Postulandi.</strong><p className="mt-1">O valor estimado ({fmt(calc.total)}) ultrapassa 20 salários mínimos (R$ {LIMITE_JEC.toLocaleString("pt-BR")}). Procure um advogado ou Defensoria Pública.</p></div></div></div>) : calc.total > 0 ? (<div className="mb-4 p-4 rounded-lg border border-green-300 bg-green-50 text-green-800 text-sm"><div className="flex items-start gap-2"><CheckCircle2 size={18} className="shrink-0 mt-0.5" /><div><strong>Valor dentro do limite para Jus Postulandi.</strong><p className="mt-1">O valor estimado ({fmt(calc.total)}) está dentro do limite de 20 salários mínimos para atuação sem advogado no JEC.</p></div></div></div>) : null}
            ...
            {tab === "peticao" && (<div>
              ...
              <div className="space-y-1 mb-5">{ARGUMENTOS_CONDICIONAIS.map((a) => (<button key={a.id} onClick={() => toggleArg(a.id)} className="w-full flex items-start gap-3 text-left px-3 py-2 rounded-lg hover:bg-[#F2EFE6]">{argsCondSel[a.id] ? <CheckCircle2 size={18} className="shrink-0 mt-0.5" style={{ color: "#3F6B4A" }} /> : <Circle size={18} className="shrink-0 mt-0.5" style={{ color: "#B9B2A0" }} />}<span className="text-sm" style={{ color: INK }}>{a.label}</span></button>))}</div>
              <div className="mt-5 mb-4"><DisclaimerBar compact /></div>
              <label className="flex items-start gap-2 text-sm mb-4 cursor-pointer" onClick={() => setAckDisclaimer(!ackDisclaimer)}>{ackDisclaimer ? <CheckCircle2 size={18} className="shrink-0 mt-0.5" style={{ color: "#3F6B4A" }} /> : <Circle size={18} className="shrink-0 mt-0.5" style={{ color: "#B9B2A0" }} />}<span style={{ color: INK }}>Li o aviso acima e entendo que devo revisar o texto, prazos e normas antes de protocolar.</span></label>
              <button onClick={gerarPeticao} disabled={!ackDisclaimer || limitReached} className="inline-flex items-center gap-1 text-sm font-medium px-4 py-2 rounded-lg text-white disabled:opacity-40 disabled:cursor-not-allowed" style={{ backgroundColor: INK }}>{limitReached ? "Limite Atingido" : "Gerar Rascunho da Petição"} <ChevronRight size={16} /></button>
            </div>)}
            {tab === "resultado" && (<div>
              <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold" style={{ color: INK }}>Rascunho Gerado</h2><div className="flex gap-2"><button onClick={copiar} className="inline-flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg border" style={{ borderColor: PAPER_LINE, color: INK }}><Copy size={14} /> Copiar Texto</button><button onClick={baixarPDF} className="inline-flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg text-white" style={{ backgroundColor: INK }}><Download size={14} /> Baixar PDF Formatado</button></div></div>
              <div className="rounded-lg border p-6 whitespace-pre-wrap text-sm leading-relaxed max-h-[600px] overflow-y-auto mb-4" style={{ backgroundColor: "#FFFDF8", borderColor: PAPER_LINE, color: "#2A2A28", fontFamily: "Georgia, 'Times New Roman', serif" }}>{geradoTexto}</div>
              <div className="p-4 rounded-lg bg-blue-50 border border-blue-200 text-blue-800 text-sm"><strong>💡 Dica Profissional:</strong> Clique em <strong>"Baixar PDF Formatado"</strong> para obter o documento com margens, fonte Times New Roman, texto justificado e espaçamento 1.5, pronto para ser impresso ou anexado no PJe.</div>
            </div>)}
          </div>
        </div>
      </div>
